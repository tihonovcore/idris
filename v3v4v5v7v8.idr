...
lemma : (x, y: Nat) -> (S x) = (S y) -> x = y
lemma y y Refl = Refl

lemma' : (x: Nat) -> x * x = 0 -> x = 0
lemma' Z     prf = Refl
lemma' (S _) Refl impossible

p3_lemma : (x : Nat) -> x + 1 = 1 -> x = 0
p3_lemma x prf = 
...
p3 : (x : Nat) -> x * x + 1 = 1 -> x = 0
p3 x prf = 
  let prev = p3_lemma (x * x) prf in 
    lemma' x prev

p4 : (LT x 3) -> Either (x=Z) (Either (x=(S Z)) (x = (S (S Z))))
...
p7 : (x: Nat) -> (GTE x (S Z)) -> (LT x (S (S Z))) -> x = (S Z)
p7 (S Z) (LTESucc LTEZero) (LTESucc (LTESucc q)) = Refl

cm : x = y -> y = x
cm Refl = Refl

p8_lemma : x = y -> LT x 3 -> LT y 3
p8_lemma eq px = replace {P = \w => LT w 3} eq px

p8 : (x: Nat) -> Either (x = Z) (Either (x = (S Z)) (x = (S (S Z)))) -> LT x (S (S (S Z)))
